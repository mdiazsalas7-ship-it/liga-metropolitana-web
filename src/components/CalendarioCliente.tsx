...
export function CalendarioCliente({
  partidos, equipos, categoriaLabel,
}: {
  partidos: Partido[]; equipos: Equipo[]; categoriaLabel: string;
}) {
        ...
        {filters.map(f => {
          ...
          return (
            <button
              ...
              className={
                'whitespace-nowrap rounded-full px-4 py-1.5 text-xs font-bold transition-colors flex items-center gap-1.5 border ' +
                (active
                  ? (isLive
            ...
            >
              ...
              {isLive && liveCount > 0 && (
                <span className={
                  ...
                  (active ? 'bg-white text-liga-live' : 'bg-liga-live text-white')
                }>
                  {liveCount}
                </span>
              )}
