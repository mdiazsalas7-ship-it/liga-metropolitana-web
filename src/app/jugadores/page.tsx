...
export const metadata: Metadata = {
  title: 'Estadísticas',
  description: 'Estadísticas y líderes de la Liga Metropolitana Eje Este.',
};

export default function JugadoresLandingPage() {
  return (
    <div className="space-y-6">
      <p className="text-xs text-[var(--color-text-dim2)]">
        <Link href="/" className="hover:text-[var(--color-text-dim)]">Inicio</Link>
        <span className="mx-1.5">›</span>
        <span className="text-[var(--color-text-dim)]">Estadísticas</span>
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {CATEGORIAS.map(c => (
          <Link key={c.id} href={`/jugadores/${c.id}`}
            ...
            <p className="text-xs text-liga-orange mt-2">Ver líderes →</p>
          </Link>
        ))}
      </div>
    </div>
